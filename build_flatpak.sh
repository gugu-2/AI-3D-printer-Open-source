...
# QuantumForge Flatpak Build Script
# This script builds and packages QuantumForge as a Flatpak package locally
...
show_help() {
    ...
    echo ""
    echo "Build QuantumForge as a Flatpak package"
    echo ""
    echo "Options:"
...
echo -e "${BLUE}QuantumForge Flatpak Build Script${NC}"
echo -e "${BLUE}================================${NC}"
...
if [[ ! -f "./scripts/flatpak/com.quantumforge.QuantumForge.yml" ]]; then
    echo -e "${RED}Error: Flatpak manifest not found at scripts/flatpak/com.quantumforge.QuantumForge.yml${NC}"
    exit 1
fi
...
BUNDLE_NAME="QuantumForge-Linux-flatpak_${VER}_${ARCH}.flatpak"
...
MANIFEST="scripts/flatpak/com.quantumforge.QuantumForge.yml"
if [[ "$NO_DEBUGINFO" == true ]]; then
    MANIFEST="scripts/flatpak/com.quantumforge.QuantumForge.no-debug.yml"
    sed '/^build-options:/a\  no-debuginfo: true\n  strip: true' \
        scripts/flatpak/com.quantumforge.QuantumForge.yml > "$MANIFEST"
...
if ! flatpak-builder \
    ...
    rm -f "scripts/flatpak/com.quantumforge.QuantumForge.no-debug.yml"
    exit 1
fi
...
echo ""
echo -e "${BLUE}To run QuantumForge:${NC}"
echo -e "flatpak run com.quantumforge.QuantumForge"
echo ""
echo -e "${BLUE}To uninstall:${NC}"
echo -e "flatpak uninstall --user com.quantumforge.QuantumForge"
echo ""
if [[ "$FORCE_CLEAN" != true ]]; then
