...
function Resolve-PluginCacheDir([string]$Dir, [string]$Current) {
    ...
    if (-not $env:APPDATA) { throw 'APPDATA is not available' }
    return [System.IO.Path]::GetFullPath((Join-Path $env:APPDATA 'QuantumForge\ota\plugins'))
}
...
if ([string]::IsNullOrWhiteSpace($PluginDir)) {
    ...
    $PluginDir = Join-Path $env:APPDATA 'QuantumForge\plugins'
}
...
Write-Host 'Now start QuantumForge.'
