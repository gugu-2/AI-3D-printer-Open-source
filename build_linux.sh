...
function get_docker_runner_image() {
    ...
    echo "quantumforge-linux-builder:${sanitized_base_image}-cmake-${sanitized_cmake_version}-${recipe_hash}"
}
...
COPY build_linux.sh /tmp/quantumforge/build_linux.sh
COPY scripts/linux.d /tmp/quantumforge/scripts/linux.d

WORKDIR /tmp/quantumforge
...
RUN rm -rf /var/lib/apt/lists/* /tmp/quantumforge
EOF
}
...
function run_in_docker() {
    ...
    host_gid=$(id -g)
    host_user="${USER:-orca}"
    container_workspace="/__w/QuantumForge/QuantumForge"
    build_args=()
...
function create_builder_user() {
    ...
    echo "${HOST_USER} ALL=(ALL) NOPASSWD:ALL" >/etc/sudoers.d/quantumforge-builder
    chmod 0440 /etc/sudoers.d/quantumforge-builder
}
...
if [[ -n "${BUILD_ORCA}" ]] || [[ -n "${BUILD_TESTS}" ]] ; then
	...
	echo "Building QuantumForge ..."
	print_and_run cmake --build $BUILD_DIR --config "${BUILD_CONFIG}" --target QuantumForge
	echo "Building QuantumForge_profile_validator .."
	print_and_run cmake --build $BUILD_DIR --config "${BUILD_CONFIG}" --target QuantumForge_profile_validator
