#!/usr/bin/env bash
#
# Build QuantumForge Flatpak locally using Docker with the same container image
...
echo "=== QuantumForge Flatpak Build ==="
...
# ---------- prepare manifest ----------
MANIFEST_SRC="scripts/flatpak/com.quantumforge.QuantumForge.yml"
MANIFEST_DOCKER="scripts/flatpak/com.quantumforge.QuantumForge.docker.yml"
...
flatpak-builder $FORCE_CLEAN_FLAG \
    ...
    --repo=flatpak-repo \
    flatpak-build \
    scripts/flatpak/com.quantumforge.QuantumForge.docker.yml
builder_end=$(date +%s)
