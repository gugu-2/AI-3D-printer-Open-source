    ...
    if (!ticket.empty()) {
        ...
        if (token_result == 0 && profile_result == 0) {
            ...
            const std::string message = login_ok
                ? "You can return to QuantumForge. This window will close automatically."
                : "Something went wrong. Please return to QuantumForge and try again.";
        ...
        const std::string html =
            ...
            "<p>Something went wrong. Please return to QuantumForge and try again.</p>"
            "</body></html>";
        return std::make_shared<ResponseHtml>(html);
    }
...
void HttpServer::ResponseRedirect::write_response(std::stringstream& ssOut)
{
    const std::string sHTML =
        ...
        "<p>You can return to QuantumForge. If your browser does not redirect automatically, use the button below.</p>"
