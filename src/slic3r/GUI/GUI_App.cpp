...
static void migrate_flatpak_legacy_datadir(const boost::filesystem::path &data_dir_path)
{
    ...
    boost::replace_first(legacy_data_dir_str, "com.quantumforge.QuantumForge", "io.github.quantumforge.QuantumForge");
...
/*#ifdef WIN32
    ...
    try {
        ...
        flush_logs();
        wxString errmsg = wxString::Format(_L("QuantumForge will terminate because of running out of memory. "
        ...
        //throw;
     	...
     	BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        	flush_logs();
        wxString errmsg = _L("QuantumForge will terminate because of a localization error. "
        ...
        BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        flush_logs();
        wxLogError(format_wxstr(_L("QuantumForge got an unhandled exception: %1%"), ex.what()));
        throw;
    }
//#endif
}
...
{
	//app config initializes early becasuse it is used in instance checking in QuantumForge.cpp
...
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t start = body.find("QuantumForge ");
    if (start == std::string::npos) {
        start = body.find("QuantumForge ");
        if (start == std::string::npos)
            return boost::none;
    }
...
//	SetAppDisplayName(SLIC3R_APP_NAME);
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("gui mode, Current QuantumForge Version %1% build %2%") % SoftFever_VERSION % GIT_COMMIT_HASH;
        ...
        if (!msg.empty() && !ssl_accept) {
            RichMessageDialog
                dlg(nullptr,
                    ...
                    "QuantumForge", wxICON_QUESTION | wxYES_NO);
...
//#endif
    ...
    if (m_config_corrupted) {
        ...
        show_error(nullptr,
                   _u8L(
                       "The QuantumForge configuration file may be corrupted and cannot be parsed.\nQuantumForge has attempted to recreate the "
                                                            ...
                                                            const boost::filesystem::path& plugin_cache_dir)
{
    ...
    wxMessageDialog prompt(
        nullptr,
        _L("The macOS Linux bridge runtime is not ready. QuantumForge can install or repair the bundled Lima runtime now."),
        _L("QuantumForge Linux Bridge"),
...
bool GUI_App::process_network_msg(std::string dev_id, std::string msg)
{
    if (dev_id.empty()) {
        if (msg == "wait_info") {
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("Please try updating QuantumForge and then try again."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("The certificate has expired. Please check the time settings or update QuantumForge and try again."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("Internal error. Please try upgrading the firmware and QuantumForge version. If the issue persists, contact support."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            MessageDialog
                msg_dlg(nullptr,
                        _L("To use QuantumForge with Bambu Lab printers, you need to enable LAN mode and Developer mode on your printer.\n\n"
                           "Please go to your printer's settings and:\n"
                           "1. Turn on LAN mode\n"
                           "2. Enable Developer mode\n\n"
                           "Developer mode allows the printer to work exclusively through local network access, "
                           "enabling full functionality with QuantumForge."),
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (initial) {
        ...
        if (! language.empty())
        	BOOST_LOG_TRIVIAL(info) << boost::format("language provided by QuantumForge.conf: %1%") % language;
        else {
	...
	if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
    	BOOST_LOG_TRIVIAL(trace) << boost::format("The following language code requires right to left layout, which is not supported by QuantumForge: %1%") % language_info->CanonicalName.ToUTF8().data();
		language_info = nullptr;
	}
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening PrusaSlicer by drag & dropping a G-Code onto QuantumForge icon in Finder,
...
void GUI_App::associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id = L" Orca.Slicer.1";
    std::wstring prog_desc = L"QuantumForge";
