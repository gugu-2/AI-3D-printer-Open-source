...
#ifdef __APPLE__
class QuantumForgeTaskBarIcon : public wxTaskBarIcon
{
public:
    QuantumForgeTaskBarIcon(wxTaskBarIconType iconType = wxTBI_DEFAULT_TYPE) : wxTaskBarIcon(iconType) {}
...
#else // _WIN32
    return wxIcon(Slic3r::var("QuantumForge_128px.png"), wxBITMAP_TYPE_PNG);
#endif // _WIN32
}
...
*/
    ...
    case GUI_App::EAppMode::Editor:
        m_taskbar_icon = std::make_unique<QuantumForgeTaskBarIcon>(wxTBI_DOCK);
        m_taskbar_icon->SetIcon(wxIcon(Slic3r::var("QuantumForge-mac_256px.ico"), wxBITMAP_TYPE_ICO), "QuantumForge");
        break;
...
// See https://github.com/QuantumForge/QuantumForge/issues/8152
...
static wxMenu* generate_help_menu()
{
    ...
    // Report a bug
    //append_menu_item(helpMenu, wxID_ANY, _L("Report Bug(TODO)"), _L("Report a bug of QuantumForge"),
    //    [](wxCommandEvent&) {
    //        //TODO
    //    });
    // Check New Version
...
void MainFrame::init_menubar_as_editor()
{
    ...
    //auto preference_item = new wxMenuItem(parent_menu, QuantumForgeMenuPreferences + bambu_studio_id_base, _L("Preferences") + "\t" + ctrl + ",", "");
#else
...
//    });
...
#ifdef __APPLE__
    wxString about_title = wxString::Format(_L("&About %s"), SLIC3R_APP_FULL_NAME);
    //auto about_item = new wxMenuItem(parent_menu, QuantumForgeMenuAbout + bambu_studio_id_base, about_title, "");
        //parent_menu->Bind(wxEVT_MENU, [this, bambu_studio_id_base](wxEvent& event) {
        //    switch (event.GetId() - bambu_studio_id_base) {
        //        case QuantumForgeMenuAbout:
        //            Slic3r::GUI::about();
        //            break;
        //        case QuantumForgeMenuPreferences:
        //            CallAfter([this] {
        //                PreferencesDialog dlg(this);
        //                dlg.ShowModal();
        //#if ENABLE_GCODE_LINES_ID_IN_H_SLIDER
    ...
    append_menu_item(m_topbar->GetCalibMenu(), wxID_ANY, _L("Calibration Guide"), _L("Calibration Guide"), [this](wxCommandEvent &)
                     { wxLaunchDefaultBrowser("https://www.quantumforge.com/wiki/calibration_guide", wxBROWSER_NEW_WINDOW); }, "", nullptr, [this]()
    ...
    append_menu_item(calib_menu, wxID_ANY, _L("Calibration Guide"), _L("Calibration Guide"),
        [this](wxCommandEvent&) { wxLaunchDefaultBrowser("https://www.quantumforge.com/wiki/calibration_guide", wxBROWSER_NEW_WINDOW); }, "", nullptr,
        [this]() {return m_plater->is_view3D_shown();; }, this);
    ...
    // Print History
    append_menu_item(tools_menu, wxID_ANY,
        _L("Print History") + "\tCtrl+H",
        _L("View print history, analytics, and export CSV"),
        [this](wxCommandEvent&) {
            PrintHistoryDialog dlg(this);
            dlg.ShowModal();
        });

    // Filament Cost Settings
    append_menu_item(tools_menu, wxID_ANY,
        _L("Filament Cost Settings"),
        _L("Configure filament cost per kg for print cost estimation"),
        [this](wxCommandEvent&) {
            FilamentCostDialog dlg(this);
            dlg.ShowModal();
        });

    // Time-lapse Injector
    append_menu_item(tools_menu, wxID_ANY,
        _L("Time-lapse G-code Injector"),
        _L("Configure automatic time-lapse G-code injection at layer changes"),
        [this](wxCommandEvent&) {
            TimelapseSettingsDialog dlg(this);
            dlg.ShowModal();
        });

    tools_menu->AppendSeparator();

    // Quick Quality Presets submenu
    ...
    static const QualityItem quality_items[] = {
        { QuickQualityLevel::Draft,     _L("Draft (0.3 mm)"),      _L("Fast print, 0.3 mm layers, high speed") },
        { QuickQualityLevel::Standard,  _L("Standard (0.2 mm)"),   _L("Balanced quality and speed, 0.2 mm layers") },
        { QuickQualityLevel::Fine,      _L("Fine (0.15 mm)"),      _L("High quality, 0.15 mm layers, reduced speed") },
        { QuickQualityLevel::UltraFine, _L("Ultra Fine (0.08 mm)"),_L("Maximum quality, 0.08 mm layers, slow speed") },
        { QuickQualityLevel::Custom,    _L("Custom (no override)"), _L("Remove quick quality override, use preset values") },
    };
    for (const auto& qi : quality_items) {
        append_menu_item(quality_menu, wxID_ANY, qi.label, qi.tooltip,
            [this, level = qi.level](wxCommandEvent&) {
                if (!m_plater) return;
                auto& mgr = QuickSettingsPresetsManager::instance();
                DynamicPrintConfig* cfg = wxGetApp().preset_bundle->prints.get_edited_preset().config.get();
                if (cfg) {
                    mgr.apply_level(level, *cfg);
                    // Trigger UI refresh
                    wxGetApp().plater()->on_config_change(*cfg);
                    BOOST_LOG_TRIVIAL(info) << "QuickPresets: applied "
                        << QuickSettingsPresetsManager::level_name(level);
                }
            });
    }
    append_submenu(tools_menu, quality_menu, wxID_ANY,
                   _L("Quick Quality Presets"), "");

    m_menubar->Append(tools_menu, wxString::Format("&%s", _L("Tools")));
    // ─────────────────────────────────────────────────────────────────────────
...
#else
    SetIcon(wxIcon(var("QuantumForge_128px.png"), wxBITMAP_TYPE_PNG));
#endif // _WIN32
