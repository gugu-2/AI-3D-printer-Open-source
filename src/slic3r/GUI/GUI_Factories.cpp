...
void MenuFactory::create_extra_object_menu()
{
    ...
    append_menu_item_apply_object_print_preset(&m_object_menu);
    m_object_menu.AppendSeparator();
...
void MenuFactory::append_menu_item_apply_object_print_preset(wxMenu* menu)
{
    const std::vector<wxString> names = { _L("Apply print preset to object…"),
                                          _L("Copy all options from a saved print preset into the selected object (speed, layer height, infill, etc.)") };
    for (const wxString& name : names) {
        const int item_id = menu->FindItem(name);
        if (item_id != wxNOT_FOUND)
            menu->Destroy(item_id);
    ...
    append_menu_item(menu, wxID_ANY, names[0], names[1],
        [](wxCommandEvent&) {
            if (printer_technology() != ptFFF)
                return;
            const int oi = obj_list()->get_selected_obj_idx();
            if (oi < 0)
                return;
            ModelObject* mo = obj_list()->object(oi);
            if (!mo)
                return;
            PresetBundle& bundle = *wxGetApp().preset_bundle;
            wxArrayString choices;
            std::vector<std::string> ordered;
            for (const Preset& pr : bundle.prints.get_presets()) {
                if (!pr.is_visible || !pr.is_compatible)
                    continue;
                choices.Add(wxString::FromUTF8(pr.name));
                ordered.push_back(pr.name);
            }
            if (choices.IsEmpty()) {
                MessageDialog dlg(plater(), _L("No compatible print presets are available."), _L("Apply print preset"), wxOK | wxICON_INFORMATION);
                dlg.ShowModal();
                return;
            }
            wxSingleChoiceDialog dlg(plater(), _L("Select a print preset to apply to this object."), names[0], choices);
            if (dlg.ShowModal() != wxID_OK)
                return;
            const int sel = dlg.GetSelection();
            if (sel < 0 || sel >= int(ordered.size()))
                return;
            std::string err;
            if (!apply_named_print_preset_to_model_object(bundle, *mo, ordered[size_t(sel)], err)) {
                MessageDialog mdlg(plater(), wxString::FromUTF8(err), _L("Apply print preset"), wxOK | wxICON_WARNING);
                mdlg.ShowModal();
                return;
            }
            obj_list()->changed_object(oi);
        },
        "", nullptr,
        []() {
            if (printer_technology() != ptFFF)
