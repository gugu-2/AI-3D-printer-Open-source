...
int CLI::run(int argc, char **argv)
{
    ...
    set_current_thread_name("quantumforge_main");
    ...
    if (start_gui) {
        ...
        BOOST_LOG_TRIVIAL(info) << "begin to launch QuantumForge GUI soon";
    ...
    BOOST_LOG_TRIVIAL(warning) << boost::format("cli mode, Current QuantumForge Version %1%")%SoftFever_VERSION;
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into QuantumForge process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into a QuantumForge process, it happens at the very start of the application,
    // thus we shall detect them now.
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the QuantumForge process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"QuantumForge is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using QuantumForge.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes QuantumForge "
...
extern "C" {
    __declspec(dllexport) int __stdcall quantumforge_main(int argc, wchar_t **argv)
    {
