...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //BBS: QuantumForge is also "bambu". Otherwise the time estimation didn't work.
    //FIXME: Workaround and should be handled when do removing-bambu
    { EProducer::QuantumForge, SLIC3R_APP_NAME },
    { EProducer::QuantumForge, "generated by QuantumForge" },
    { EProducer::QuantumForge, "generated by BambuStudio" },
    { EProducer::QuantumForge, "BambuStudio" }
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by QuantumForge,
        // extract the config from it
        if (m_producer == EProducer::QuantumForge || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from QuantumForge's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::QuantumForge) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::QuantumForge: { return process_bambuslicer_tags(comment); }
...
// BambuStudio used extruder_id*2+mode to support per-nozzle limits, but QuantumForge
