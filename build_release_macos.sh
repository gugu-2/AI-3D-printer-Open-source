...
build_deps() {
    for _ARCH in x86_64 arm64; do
        if [ "$ARCH" == "universal" ] || [ "$ARCH" == "$_ARCH" ]; then
            ...
            DEPS="$DEPS_BUILD_DIR/QuantumForge_dep"
...
pack_deps() {
    ...
    (
        set -x
        cd "$DEPS_DIR"
        tar -zcvf "QuantumForge_dep_mac_${ARCH}_$(date +"%Y%m%d").tar.gz" "build"
    )
}
...
build_slicer() {
    for _ARCH in x86_64 arm64; do
        if [ "$ARCH" == "universal" ] || [ "$ARCH" == "$_ARCH" ]; then
            ...
            (
                cd "$PROJECT_BUILD_DIR"
                mkdir -p QuantumForge
                cd QuantumForge
                rm -rf ./QuantumForge.app
                cp -pR "../src$BUILD_DIR_CONFIG_SUBDIR/QuantumForge.app" ./QuantumForge.app

                resources_path=$(readlink ./QuantumForge.app/Contents/Resources)
                rm ./QuantumForge.app/Contents/Resources
                cp -R "$resources_path" ./QuantumForge.app/Contents/Resources

                find ./QuantumForge.app/ -name '.DS_Store' -delete

                if [ -f "../src$BUILD_DIR_CONFIG_SUBDIR/QuantumForge_profile_validator.app/Contents/MacOS/QuantumForge_profile_validator" ]; then
                    echo "Copying QuantumForge_profile_validator.app..."
                    rm -rf ./QuantumForge_profile_validator.app
                    cp -pR "../src$BUILD_DIR_CONFIG_SUBDIR/QuantumForge_profile_validator.app" ./QuantumForge_profile_validator.app
                    find ./QuantumForge_profile_validator.app/ -name '.DS_Store' -delete
                fi

                copy_linux_bridge_runtime_to_app "./QuantumForge.app" "$PROJECT_BUILD_DIR/QuantumForge"
            )
        fi
    done
}
...
build_universal() {
    ...
    PROJECT_BUILD_DIR="$PROJECT_DIR/build/$ARCH"
    ARM64_APP="$PROJECT_DIR/build/arm64/QuantumForge/QuantumForge.app"
    X86_64_APP="$PROJECT_DIR/build/x86_64/QuantumForge/QuantumForge.app"

    mkdir -p "$PROJECT_BUILD_DIR/QuantumForge"
    UNIVERSAL_APP="$PROJECT_BUILD_DIR/QuantumForge/QuantumForge.app"
    rm -rf "$UNIVERSAL_APP"
    cp -R "$ARM64_APP" "$UNIVERSAL_APP"

    echo "Creating universal binaries for QuantumForge.app..."
    lipo_dir "$UNIVERSAL_APP" "$X86_64_APP"
    echo "Universal QuantumForge.app created at $UNIVERSAL_APP"

    ARM64_VALIDATOR="$PROJECT_DIR/build/arm64/QuantumForge/QuantumForge_profile_validator.app"
    X86_64_VALIDATOR="$PROJECT_DIR/build/x86_64/QuantumForge/QuantumForge_profile_validator.app"
    if [ -d "$ARM64_VALIDATOR" ] && [ -d "$X86_64_VALIDATOR" ]; then
        echo "Creating universal binaries for QuantumForge_profile_validator.app..."
        UNIVERSAL_VALIDATOR_APP="$PROJECT_BUILD_DIR/QuantumForge/QuantumForge_profile_validator.app"
        ...
        echo "Universal QuantumForge_profile_validator.app created at $UNIVERSAL_VALIDATOR_APP"
    fi
}
